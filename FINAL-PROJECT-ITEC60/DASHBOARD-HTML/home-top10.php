...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_episode_id'])) {
  ...
  mysqli_query($con, "DELETE FROM tbl_top10_episodes WHERE episode_id = $episode_id");
  echo 'success';
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_season_id'])) {
  ...
  mysqli_query($con, "DELETE FROM tbl_top10_episodes WHERE season_id = $season_id");
  // Delete the season
  mysqli_query($con, "DELETE FROM tbl_top10_seasons WHERE season_id = $season_id");
  echo 'success';
  exit;
}
...
if ($is_edit) {
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    if ($record['category'] === 'Series') {
      // Fetch seasons
      $seasonq = mysqli_query($con, "SELECT * FROM tbl_top10_seasons WHERE top10_id = $top10_id ORDER BY season_number");
      while ($season = mysqli_fetch_assoc($seasonq)) {
        // Fetch episodes for this season
        $epq = mysqli_query($con, "SELECT * FROM tbl_top10_episodes WHERE season_id = {$season['season_id']} ORDER BY episode_id");
        $season['episodes'] = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['delete_episode_id']) && !isset($_POST['delete_season_id'])) {
  ...
  if (
    ...
    if ($is_edit) {
      ...
      $saved_top10_id = mysqli_insert_id($con);
    }
    ...
    if (!$query) {
      ...
      if ($is_series) {
        ...
        if ($is_edit) {
          $seasonidsq = mysqli_query($con, "SELECT season_id FROM tbl_top10_seasons WHERE top10_id = $saved_top10_id");
          while ($row = mysqli_fetch_assoc($seasonidsq)) {
            $sid = $row['season_id'];
            mysqli_query($con, "DELETE FROM tbl_top10_episodes WHERE season_id = $sid");
          }
          mysqli_query($con, "DELETE FROM tbl_top10_seasons WHERE top10_id = $saved_top10_id");
        }
        foreach ($season_titles as $sidx => $season_title) {
          $season_title_esc = mysqli_real_escape_string($con, $season_title);
          $season_num = intval($season_numbers[$sidx]);
          mysqli_query($con, "INSERT INTO tbl_top10_seasons (top10_id, season_number, season_title) VALUES ($saved_top10_id, $season_num, '$season_title_esc')");
          ...
          for ($eidx = 0; $eidx < $max_eidx; $eidx++) {
            ...
            if ($video !== '' && isset($video_tmps[$eidx]) && $video_tmps[$eidx]) {
              ...
              $ep_video_path = 'TOP10_EPISODES/' . $ep_video_name;
            ...
            $ep_sql = "INSERT INTO tbl_top10_episodes (season_id, episode_title, episode_description, episode_duration, episode_video) VALUES ($season_id, '$ep_title_esc', '$ep_desc_esc', '$ep_duration_esc', '$ep_video_name')";
...
<body>
  <main class="container-lg p-0 overflow-hidden">
          ...
          <form method="post" enctype="multipart/form-data" id="top10-form">
            ...
            <div class="d-flex justify-content-end mb-3">
              <button type="submit" name="submit-btn"
                class="btn db-bg-primary text-white ms-3 mt-3"><?php echo $is_edit ? 'Update' : 'Add'; ?> Top
                10</button>
            </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete-episode-btn-modal"])) {
      ...
      $delete_query = "DELETE FROM tbl_top10_episodes WHERE episode_id = $delete_id";
