...
if ($is_edit) {
  $result = mysqli_query($con, "SELECT * FROM tbl_movie_series WHERE movie_id = $movie_id LIMIT 1");
  if ($result && mysqli_num_rows($result) > 0) {
    $record = mysqli_fetch_assoc($result);
    if ($record['category'] === 'Series') {
      // Fetch seasons
      $seasonq = mysqli_query($con, "SELECT * FROM tbl_movie_series_seasons WHERE movie_id = $movie_id ORDER BY season_number");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['delete_episode_id']) && !isset($_POST['delete_season_id'])) {
  ...
  if (isset($_FILES['poster']) && $_FILES['poster']['error'] === UPLOAD_ERR_OK) {
    ...
    $file_poster_path = 'MSERIES_IMAGES/' . $file_poster;
  ...
  if (isset($_FILES['video']) && $_FILES['video']['error'] === UPLOAD_ERR_OK && !is_array($_FILES['video']['name'])) {
    ...
    $file_video_path = 'MSERIES_VIDEOS/' . $file_video;
  ...
  if (isset($_FILES['modal-poster-title']) && $_FILES['modal-poster-title']['error'] === UPLOAD_ERR_OK) {
    ...
    $file_modal_poster_title_path = 'MSERIES_IMAGES/' . $file_modal_poster_title;
  ...
  if (
    ...
    if ($is_edit) {
      ...
      $saved_movie_id = mysqli_insert_id($con);
    }
    ...
    if (!$query) {
      ...
      if ($is_series) {
        ...
        if ($is_edit) {
          $seasonidsq = mysqli_query($con, "SELECT season_id FROM tbl_movie_series_seasons WHERE movie_id = $saved_movie_id");
          while ($row = mysqli_fetch_assoc($seasonidsq)) {
            $sid = $row['season_id'];
            mysqli_query($con, "DELETE FROM tbl_movie_series_episodes WHERE season_id = $sid");
          }
          mysqli_query($con, "DELETE FROM tbl_movie_series_seasons WHERE movie_id = $saved_movie_id");
        }
        foreach ($season_titles as $sidx => $season_title) {
          $season_title_esc = mysqli_real_escape_string($con, $season_title);
          $season_num = intval($season_numbers[$sidx]);
          mysqli_query($con, "INSERT INTO tbl_movie_series_seasons (movie_id, season_number, season_title) VALUES ($saved_movie_id, $season_num, '$season_title_esc')");
          ...
          for ($eidx = 0; $eidx < $max_eidx; $eidx++) {
            ...
            if ($video !== '' && isset($video_tmps[$eidx]) && $video_tmps[$eidx]) {
              ...
              $ep_video_path = 'MSERIES_EPISODES/' . $ep_video_name;
      ...
      <div class="card bg-dark">
        <div class="card-body">
          <div class="table-task-top db-text-sec">
            <div class="py-2 ps-3">
              <p class="m-0 fs-20"><?php echo $is_edit ? 'Edit' : 'Add'; ?> Movie/Series</p>
              <p class="m-0 fs-14" style="transform: translateY(-2px)">
                <?php echo $is_edit ? 'Edit your entry in the CineVault database' : 'Add your favorite movies/series to the CineVault database here'; ?>
              </p>
            </div>
          </div>
          ...
          <form method="post" enctype="multipart/form-data" id="top10-form">
            ...
            <div class="d-flex justify-content-end mb-3">
              <button type="submit" name="submit-btn"
                class="btn db-bg-primary text-white ms-3 mt-3"><?php echo $is_edit ? 'Update' : 'Add'; ?> Movie/Series</button>
            </div>
