      ...
      <div class="signin-container">
        ...
        if (isset($_POST["signup-btn"])) {
          ...
          if ($valid_email && $valid_pass) {
            ...
            $stmt = $con->prepare("INSERT INTO tbl_signup_acc (signup_email, signup_password) VALUES (?, ?)");
            $stmt->bind_param("ss", $email, $hash_pass);
            $stmt->execute();
            $signup_id = $stmt->insert_id;
            $stmt->close();

            // Save to session for payment step
            $_SESSION['signup_id'] = $signup_id;
            $_SESSION['signup_email'] = $email; // (Optional, for later use)
            // Never save password hash in session for security
