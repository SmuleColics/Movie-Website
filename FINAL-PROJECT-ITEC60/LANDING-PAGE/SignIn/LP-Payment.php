...
if (isset($_POST['confirm'])) {
  ...
  if (isset($_SESSION['signup_email']) && isset($_SESSION['signup_password']) && $valid_reference) {
    $email = $_SESSION['signup_email'];
    $password = $_SESSION['signup_password'];

    $insert_signup = mysqli_query($con, "INSERT INTO tbl_signup_acc (signup_email, signup_password) VALUES ('$email', '$password')");

    // Get the new ID
    $signup_id = mysqli_insert_id($con);

    $insert_payment = mysqli_query($con, "INSERT INTO tbl_payment(signup_id, reference_no) VALUES($signup_id, '$reference_no')");
    echo "<script>alert('Payment successful! Account created.'); window.location.href='LP-SignIn.php';</script>";
    session_unset();
    session_destroy();
    exit; 
  }
      ...
      <main>
        <section class="position-relative">
          ...
          <div id="payment-container" class="signin-container db-text-sec">
            ...
            <h1 class="signin-text text-start mb-4 text-center" style="padding: 0; margin-top: 30px;">Complete Your Payment</h1>
              ...
              <div class="d-grid ">
                ...
                <div id="reference-container" class="d-none">
                  <div class="d-grid">
                    ...
                    <input class="border-bottom border-0 bg-transparent db-text-sec input-payment" 
                    type="text" 
                    name="reference-no" 
                    id="reference-no" 
                    placeholder="0"
                    autocomplete="off"
                    >
                    <p id="ref-dup" class="text-start mt-1 mb-0 text-danger" style="font-size: 14px; <?php if (!(isset($_POST['confirm']) && !$valid_reference)) { echo 'display: none;'; } ?>">
                      <?php if (isset($_POST['confirm']) && !$valid_reference) { echo $reference_error; } ?>
                    </p>
                    ...
                    <input type="submit" class="btn db-text-sec db-bg-primary my-4 button-payment" name="confirm" id="confirm" value="Confirm">
                  </div>
                </div>
